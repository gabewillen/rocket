...
class AdaptiveMtpPolicyTests(unittest.TestCase):
    ...
    def test_c16_k4_is_lazy_and_requires_live_promotion_evidence(self) -> None:
        config = matched_live_policy_config()
        policy = AdaptiveMtpPolicy(Tracer(), config)
        state = policy.initial_state()
        for _ in PROBE_DEPTHS:
            ...
            self.assertEqual(decision.selected_depth, MtpDepth.K1)
            state = decision.next_state

        def efficient(depth: MtpDepth, step_bytes: int) -> PolicyObservation:
            return PolicyObservation(
                16,
                depth,
                (9_900,) * int(depth),
        ...
        estimates = tuple(
            DepthEstimate(efficient(depth, 500_000_000 + int(depth) * 50_000_000), 1)
            for depth in (MtpDepth.K0, MtpDepth.K1, MtpDepth.K2, MtpDepth.K3, MtpDepth.K4)
        )
        cohort = next(item for item in state.cohorts if item.concurrency == 16)
        cohort = replace(
            cohort,
            active_depth=MtpDepth.K1,
            decode_rounds=len(PROBE_DEPTHS),
            eligible_rounds=17,
            position_successes=(9_900, 9_900, 9_900, 9_900, 0, 0, 0),
            position_trials=(10_000, 10_000, 10_000, 10_000, 0, 0, 0),
            estimates=estimates,
        )
        state = replace(
            state,
            ...
            cohorts=tuple(
                cohort if item.concurrency == 16 else item for item in state.cohorts
            ),
        )
        observation = efficient(MtpDepth.K4, 700_000_000)
        for _ in range(8):
            decision = policy.decide(
                state,
                PolicyEvent(
            ...
            state = decision.next_state
        self.assertEqual(decision.selected_depth, MtpDepth.K4)
        self.assertEqual(decision.reason, DecisionReason.PROMOTE)
        self.assertEqual(
            tuple((item.action, item.depth) for item in decision.residency_commands),
            (
                (ResidencyAction.LOAD, MtpDepth.K2),
                (ResidencyAction.LOAD, MtpDepth.K3),
                (ResidencyAction.LOAD, MtpDepth.K4),
            ),
        )
