...
Rocket fixed-shape fork of FlashInfer commit
91bda04c66f7cb851e1ab3b78b9fecea644b9844, source SHA-256
cfdee49ee968868dee7ce1a0b4e2b62a79e2af8990a3961178388d209627a32f.
The Qwen-only changes are D256, padded Hq16, four queries per CTA, gathered
N16 selected-token tiles, and per-token query-membership masks.
...
class Qwen38QsaPrefillSm121:
    ...
    def __init__(
        ...
        if (head_dim, m_block_size, n_block_size, group_size, num_threads) != (
            256,
            64,
            16,
            16,
            ...
            raise ValueError("Qwen QSA requires D256/M64/N16/G16/T128")
        ...
        if paged or kv_fp8 or kv_nvfp4 or kv_packed:
            raise ValueError("Qwen QSA prefill consumes flat BF16 state")
    ...
    @cute.jit
    def _load_qwen_gathered_kv(
        self,
        mK: cute.Tensor,
        mV: cute.Tensor,
        sK: cute.Tensor,
        sV: cute.Tensor,
        mUnionTokens: cute.Tensor,
        work_idx: cutlass.Int32,
        union_tile: cutlass.Int32,
        ...
        for iteration in cutlass.range_constexpr(total_chunks // self._num_threads):
            chunk = tidx + iteration * self._num_threads
            token_slot = chunk // chunks_per_row
            chunk8 = chunk % chunks_per_row
            logical = mUnionTokens[work_idx, union_tile, token_slot]
            sK_chunk = cute.local_tile(sK[token_slot, None], (8,), (chunk8,))
            sV_chunk = cute.local_tile(sV[token_slot, None], (8,), (chunk8,))
            if logical >= 0 and logical < seqlen_k:
                gK = cute.local_tile(
                    mK[k_start + logical, kv_head, None], (8,), (chunk8,)
                )
                gV = cute.local_tile(
                    mV[k_start + logical, kv_head, None], (8,), (chunk8,)
                )
                cute.autovec_copy(gK, sK_chunk)
                cute.autovec_copy(gV, sV_chunk)
            else:
                sK_chunk.fill(0)
                sV_chunk.fill(0)
    ...
    def __call__(
        ...
        mUnionBlocks: cute.Tensor,  # (capacity, max_union, 16) int32 token ids
        mUnionMasks: cute.Tensor,  # (capacity, max_union, 16) int32 query masks
        mUnionCount: cute.Tensor,  # (capacity,) int32 N16 tiles in the union
    ...
    def kernel(
        ...
        if has_work:
            ...
            cO = cute.make_identity_tensor((self._m_block_size, self._head_dim))
            ...
            for u in cutlass.range(union_count):
                ...
                for r in cutlass.range_constexpr(n_rows):
                    ...
                    for c in cutlass.range_constexpr(cute.size(tScS_mn.shape[1])):
                        token_slot = tScS_mn[0, c][1]
                        k_pos = mUnionBlocks[work_idx, u, token_slot]
                        mask_word = mUnionMasks[work_idx, u, token_slot]
                        bit = (mask_word >> tok) & cutlass.Int32(1)
                        if bit == 0 or k_pos < 0 or cute.elem_less(
                            col_limit, k_pos + 1
                        ):
            ...
            for r in cutlass.range_constexpr(n_rows):
                ...
                if cute.elem_less(q_loc, seqlen_q):
                    ...
                    inv = 0.0 if (rs == 0.0 or rs != rs) else 1.0 / rs
                    ...
                    for c in cutlass.range_constexpr(cute.size(tOcO_mn.shape[1])):
                        d_pos = tOcO_mn[0, c][1]
...
class BuildUnionMetaSm12x:
    ...
    def __init__(self, topk: int, tokens_per_tile: int):
        if (topk, tokens_per_tile) != (2051, 4):
    ...
    def _k_build(
        ...
        u = cutlass.Int32(0)
        m = cutlass.Int32(0)
        # CUTE staged loops cannot break from cutlass.range. Both m and u are
        # warp-uniform, so this condition exits every lane together at the first
        # exhausted merge step while retaining the allocation bound as a guard.
        while (m != sentinel) and (u < self._max_union):
            head = sentinel
            if active:
                if p < self._topk:
            ...
            # The reduced head is uniform across the warp. Qwen's four adjacent
            # causal lists usually exhaust near top-k, while the disjoint bound is
            # 4*top-k. The last sentinel iteration emits nothing and terminates the
            # staged while condition on the next test.
            if m != sentinel:
