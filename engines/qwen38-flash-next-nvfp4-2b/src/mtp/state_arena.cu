...
std::size_t StateArena::transaction_bytes()const noexcept{return bytes_/(depth_+1);}
PrefixStateView StateArena::bind_view(std::byte* p,std::size_t r)const noexcept{PrefixStateView v{};v.multi_hidden=take<__nv_bfloat16>(p,r*kMtpMultiHidden);v.main_key=take<__nv_bfloat16>(p,r*kQsaMainKvWidth);v.main_value=take<__nv_bfloat16>(p,r*kQsaMainKvWidth);v.raw_key=take<__nv_bfloat16>(p,r*kQsaIndexerWidth);v.compressed_key=take<__nv_bfloat16>(p,r*kQsaIndexerWidth);v.rope_positions=uses_mrope_?take<std::int64_t>(p,r*kMropeAxes):nullptr;v.main_slots=take<std::int32_t>(p,r);v.raw_slots=take<std::int32_t>(p,r);v.compressed_slots=take<std::int32_t>(p,r);v.compressed_valid=take<std::int32_t>(p,r);return v;}
void StateArena::select(const std::int32_t* w,std::uint64_t g,cudaStream_t st,std::byte* storage){if(!w||!st||!g||pending_generation_)throw StateArenaError("MTP state selection contract changed");auto destination=storage?bind_view(storage,sequences_):selected_;select_prefixes<<<sequences_,256,0,st>>>(prefixes_,destination,w,sequences_,depth_,uses_mrope_);check(cudaGetLastError(),"select complete accepted MTP prefix");pending_generation_=g;}
