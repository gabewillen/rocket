...
3. **sm_121 is outside several vendor kernel matrices.** TensorRT-LLM's FMHA
   runner rejects the architecture outright. Accelerants own their attention
...
The **compiler** runs offline on the target hardware and consumes the model
checkpoint, a hardware profile produced by M0, and an expert
...
1. **The memory problem becomes a compile-time assertion.** Section 4.1 exists
   ...
   moving target. The compiler decides residency offline, down to byte offsets, and
