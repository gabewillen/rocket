---
title: "GLM-5.3-Flash"
subtitle: "The chemistry of the first fuel, and the engine that burns it."
description: "321B total, 18B active, hybrid KDA and sparse MLA, 288 experts top-8. Two sparks, 4-bit weights."
status: designing
...
::: {.callout-note}
Chemistry, not architecture. [Design](../design.qmd) covers rocket itself:
the compiler, the engine contract, and the spark it burns on. This page
...
This engine targets the **4-bit weight regime**, with ~82 GiB resident weights per
...
**The engine treats the (KV pages, KDA state) pair as one indivisible cache
...
width lifted the same box to 32 concurrent requests. The engine treats **KDA slots
...
5. **Vision encoder scope.** In the first engine or a later one? It is 24 layers of
