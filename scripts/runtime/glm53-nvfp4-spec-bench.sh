...
common=(
  ...
  --expert-cache-gib "$EXPERT_CACHE_GIB" --max-tokens "$MAX_TOKENS" --preload-owned 1
)
...
remote=(env)
for v in ROCKET_FUEL_NVFP4_DIR ROCKET_PACKED_WEIGHTS ROCKET_FP8_ATTN_DIR ROCKET_RDMA_WAIT_S \
