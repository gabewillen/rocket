...
remote=(env)
for v in ROCKET_FUEL_NVFP4_DIR ROCKET_PACKED_WEIGHTS ROCKET_EXPERT_SLAB_DIR ROCKET_RESIDENT_DIRECT ROCKET_RDMA_WAIT_S ROCKET_KDA_CUBLAS ROCKET_CUBLAS_ALL ROCKET_DFLASH2_DIR ROCKET_MLA_LEGACY_CONTEXT ROCKET_MLA_LEGACY_SPLIT GLM53_ALLOW_LAUNCH; do
  [[ -n ${!v:-} ]] && remote+=("$v=${!v}")
done
