...
import torch
import flashinfer.msa_ops.cute_dsl.sparse_prefill_sm12x as upstream_cute
from flashinfer.msa_ops.sparse_prefill import (
    _build_union_metadata_device,
    _cutlass_dtype,
    _fake,
)
...
def pad_union(union_tokens, union_masks, union_count):
    positions = torch.arange(
        union_tokens.shape[1], dtype=torch.int32, device=union_tokens.device
    )
    valid = positions[None, :] < union_count[:, None]
    union_tokens = torch.where(valid, union_tokens, -1)
    union_masks = torch.where(valid, union_masks, 0)
    padding = (-union_tokens.shape[1]) % N_TILE
    if padding:
        union_tokens = torch.nn.functional.pad(union_tokens, (0, padding), value=-1)
        union_masks = torch.nn.functional.pad(union_masks, (0, padding), value=0)
    return (
        union_tokens.view(union_tokens.shape[0], -1, N_TILE),
        union_masks.view(union_masks.shape[0], -1, N_TILE),
        (union_count + N_TILE - 1) // N_TILE,
    )
...
def main() -> None:
    ...
    # The upstream wrapper owns capture-safe workspace construction. Route its
    # exact compile hook to Rocket's fixed top-k2051/tile4 attributed builder.
    upstream_cute.BuildUnionMetaSm12x = fork.BuildUnionMetaSm12x
    ...
    def build_metadata():
        tokens, masks, counts, work, capacity = _build_union_metadata_device(
            q2k, cu_q, TOKENS_PER_TILE, TOPK
        )
        union_token_count = counts.sum()
        tokens, masks, counts = pad_union(tokens, masks, counts)
        return tokens, masks, counts, work, capacity, union_token_count
    ...
    for _ in range(7):
        ...
        begin.record()
        measured_metadata = build_metadata()
        end.record()
        end.synchronize()
        metadata_samples.append(begin.elapsed_time(end))
    del measured_metadata
    ...
    union_tokens_total = union_token_count.item()
    ...
    print(
        ...
        f"stage_median_sum_ms="
        f"{statistics.median(kernel_samples) + statistics.median(metadata_samples)} "
