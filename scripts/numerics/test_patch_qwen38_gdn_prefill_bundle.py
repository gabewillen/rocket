...
class TestGdnPrefillBundlePatcher(unittest.TestCase):
    def test_capture_is_real_prefill_only_and_content_addressed(self):
        ...
        self.assertIn('"q": ((rows, 8, 128), torch.bfloat16)', source)
        self.assertIn('"initial_state": ((1, 24, 128, 128), torch.float32)', source)
        self.assertIn("self.prefix, query_non_spec, key_non_spec", source)
        self.assertIn('if ".layers.0." not in prefix', source)
        ...
        self.assertIn("oracle prefill chunk ended before all boundaries", source)
